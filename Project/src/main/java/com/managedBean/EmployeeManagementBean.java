		...
		if (Validate.birthdayOfEmployee(employee.getBirthday())) {
			...
			if (!userService.existUserEmail(employee.getEmail())) {
				...
				if (!userService.existUserCardId(employee.getIdCard())) {
					System.out.println("Id nuk ekzistonnnnnnnnnnnnnnnnnnnnnnnnnnn");
...
System.out.println("Metoda ne beannnnnnnnnnnnnnnnnnnnnnnn");
	...
	public String loadUser() {
		...
		if (id == 0) {
			Messages.addMessage("Give an id!", "warn");
		} else {
			UserDto user = userService.getUserFromId(id);
			if (user == null) {
				Messages.addMessage("There is no employee!", "warn");
			} else if (user.getRole().equals("manager") || user.getManagedBy() != userBean.getUser().getId()) {
				Messages.addMessage("You can't edit this employee", "warn");
			} else {
				this.employee = user;
			}
		}
